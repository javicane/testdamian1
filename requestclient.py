...
#print("in requestclient.py scope global in the module")
class RequestClient(object):
    #pdb.set_trace()
    #print(" in requestclient.RequestClient scope global in the class")
    def __init__(self, **kwargs):
        ...
        """
        #print("in requestclient.RequestClient __init__")
        api_key = None
        secret_key = None
        ...
        if "url" in kwargs:
            url = kwargs["url"]
        #print("in requestclient.RequestClient __init__ line 28")
        try:
            #print("in requestclient.RequestClient __init__ line 30")
            self.request_impl = RestApiRequestImpl(api_key, secret_key, url)
        except Exception:
            pass
        self.limits = {}
        #print("in requestclient.RequestClient __init__ linea final")
    ...
    def put_order(self, side: 'str', symbol: 'str',  
                quantity: 'float' = None,
                price: 'float' = None,
        ...
        Modify order.
        """
        print("in requestclient.RequestClient.put_order, call call_sync(self.request_impl.put_order(symbol...")
        response = call_sync(self.request_impl.put_order(side, symbol, quantity, price, origClientOrderId))
        self.refresh_limits(response[1])
        return response[0]
    ...
    def post_order(self, symbol: 'str', side: 'OrderSide', ordertype: 'OrderType', 
        ...
        Send in a new order.
        """
        print("in requestclient.RequestClient.post_order, call call_sync(self.request_impl.post_order(symbol...")
    ...
    def start_user_data_stream(self) -> any:
        ...
        """
        #pdb.set_trace()
        print("in requestclient.RequestClient.start_user_data_stream BEGIN")
        response = call_sync(self.request_impl.start_user_data_stream())
        
        print(".... in requestclient.RequestClient.start_user_data_stream after call call_sync")
        print(".... in requestclient.RequestClient.start_user_data_stream response: " + str(response))
        print(".... in requestclient.RequestClient.start_user_data_stream response is type: " + str(type(response)))
        self.refresh_limits(response[1])
        print("in requestclient.RequestClient.start_user_data_stream return response[0] END")
        return response[0]
