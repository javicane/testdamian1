...
class SubscriptionClient(object):
    #input("in binance_d.subscriptionclient scope global, pec")
    #print("in binance_d.subscriptionclient scope global, pec")
    ...
    def __init__(self, **kwargs):
        ...
        if "receive_limit_ms" in kwargs:
            ...
            print("in SubscriptionClient __init__ , parameter received receive_limit_ms:", receive_limit_ms)
        if "connection_delay_failure" in kwargs:
            connection_delay_failure = kwargs["connection_delay_failure"]
        #input("aca viene el testeo de CCC3 de la bitacora websocket_flow.txt, pec")
     
        self.__watch_dog = WebSocketWatchDog(is_auto_connect, receive_limit_ms, connection_delay_failure)
        #print("si estoy testeando CCC3 no deberia aparecer invocacion a WebSocketWatchDog")
        #input("in binance_d.subscriptionclient.SubscriptionClient.__init__, pec")
