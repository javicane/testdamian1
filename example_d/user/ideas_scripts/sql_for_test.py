...
def test():
    ...
    sql ="update r_list set order_status ='PNL' where order_id_to_close = 667"
    result = exec_sql(sql = sql , sql_command = "update", error_message = "update error message")
    print("result update is: " + str(result))
    ...
    sql = "delete from r_list where order_id_to_close = 667 and order_status='PNL'"
    result = exec_sql(sql = sql , sql_command = "delete", error_message = "error in delete")
...
sql = "CREATE TABLE if not exists order_hist ("
sql += "current_timestamp integer not null,"
sql += "activatePrice text not null,"
sql += "avgPrice text not null,"
sql += "clientOrderId text not null,"
sql += "closePosition text not null,"
sql += "cumBase text not null,"
sql += "executedQty text not null,"
sql += "orderId text not null,"
sql += "origQty text not null,"
sql += "origType text not null,"
sql += "positionSide text not null,"
sql += "price text not null,"
sql += "priceRate text not null,"
sql += "reduceOnly text not null,"
sql += "side text not null,"
sql += "status text not null,"
sql += "stopPrice text not null,"
sql += "symbol text not null,"
sql += "timeInForce text not null,"
sql += "type text not null,"
sql += "updateTime text not null,"
sql += "workingType text not null);"
print(sql)
result = exec_sql(sql = sql, sql_command = "ddl", error_message = "create table order_hist")
test()
