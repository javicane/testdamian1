...
#sqlite> 
    ...
    @patch('binance_d.example_d.user.process_module.process_pnl')
    @patch('binance_d.example_d.user.process_module.process_goto_in_position')
    def test_call_process_pnl_error(self, mock_process_goto_in_position, mock_process_pnl, mock_get_radar_list):
        mock_process_pnl.return_value = "PNL"
        my_dict = dict(order_id=1,order_status='PNL',order_id_to_close=6311530344,trigger_price_to_put_in_position=0.89,
                       trigger_price_to_close=0.893, original_quantity=97)
        my_list = []
        my_list.append(my_dict)
        mock_get_radar_list.return_value = my_list
        ...
        #rc = process_event(event_dict)              
        #self.assertIn("ERROR, cannot process_pnl, strange situation", rc)  #OK
...
# tests for rp == 0
    ...
    @patch('binance_d.example_d.user.process_module.process_pnl')
    @patch('binance_d.example_d.user.process_module.process_goto_in_position')
    def test_call_process_goto_in_position_error(self, mock_process_goto_in_position, mock_process_pnl, mock_get_radar_list):

        mock_process_pnl.return_value = "PNL"
        mock_process_goto_in_position.return_value = "GOTO_IN_POSITION"
        my_dict = dict(order_id=1,order_status='NEW',order_id_to_close=222,trigger_price_to_put_in_position=0.89,
                       trigger_price_to_close=0.893, original_quantity=97)
        my_list = []
        my_list.append(my_dict)
        mock_get_radar_list.return_value = my_list
        ...
        #rc = process_event(event_dict)              
        #self.assertEqual("ERROR, cannot process_goto_in_position, strange situation", rc)

    ######## new new 
    @patch('binance_d.example_d.user.process_module.get_radar_list')
    def test_new_order_id_to_close(self, mock_get_radar_list):
        ...
        #rc = process_event(event_dict)              
        #self.assertEqual("DETECTED new new in_position order_id_to_close , this is the creation of a new order_id_to_close", rc) #OK
    ...
    def test_initial_pivot(self, mock_process_goto_in_position, mock_process_pnl, mock_get_radar_list):
        ...
        #rc = process_event(event_dict)              
        #self.assertIn("DETECTED initial pivot", rc) #OK

    @patch('binance_d.example_d.user.process_module.get_radar_list')
    def test_new_new_strange(self, mock_get_radar_list):

        my_dict = dict(order_id=1,order_status='PNL',order_id_to_close=333,trigger_price_to_put_in_position=0.89,
                       trigger_price_to_close=0.893, original_quantity=97)
        my_list = []
        my_list.append(my_dict)
        mock_get_radar_list.return_value = my_list
        ...
        #rc = process_event(event_dict)              
        #self.assertEqual("ERROR, strange situation", rc) #OK
