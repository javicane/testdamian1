...
#}
    # event_dict: {'activationPrice': 'None', 'asksNotional': '1134.13422041', 'avgPrice': '0.0', 'bidsNotional': '100.0', 
    # 'callbackRate': 'None', 'clientOrderId': 'damian', 'commissionAmount': 'None', 'commissionAsset': 'None', 'cumulativeFilledQty': '0.0', 
    # 'eventTime': '1652672521733', 'eventType': 'ORDER_TRADE_UPDATE', 'executionType': 'NEW', 'isClosePosition': 'False', 'isMarkerSide': 'False', 
    # 'isReduceOnly': 'False', 'lastFilledPrice': '0.0', 'lastFilledQty': '0.0', 'orderId': '6429030433', 'orderStatus': 'NEW', 
    # 'orderTradeTime': '1652672521730', 'origQty': '1.0', 'positionSide': 'LONG', 'price': '0.1', 'side': 'BUY', 'stopPrice': '0.0', 
    # 'symbol': 'ADAUSD_PERP', 'timeInForce': 'GTC', 'tradeID': '0', 'transactionTime': '1652672521730', 'type': 'LIMIT', 'workingType': 'CONTRACT_PRICE'}
    #nombre reales de eventos:
    #"x":"NEW",                  // Execution Type
    #"X":"NEW",                  // Order Status
    print("in process_event BEGIN")
    event_execution_type = event_dict['o']['x'] # executionType
    event_order_status = event_dict['o']['X'] # orderStatus
    event_order_id = event_dict['o']['i'] # orderId
    event_rp = event_dict['o']['rp'] #realized profit of the trade, is != 0 when an order which was in position was executed
    event_clientorderid = event_dict['clientOrderId'] # clientOrderId
    sys.exit(66)  
...
# rp : realized profit of the trade, is != 0 when an order which was in position was executed
        ...
        print("DETECED TRADE FILLED rp == 0")
        if event_order_id in str(radar_list):
            print("OK, event_order_id exists in radar_list")
            print("check status in radar_list")
            for radar_list_dict in radar_list:
            ...
            print("ERROR , event_order_id not exists in radar_list")
            #raise
    elif event_clientorderid == "damian":
        print("DETECTED FAKE ORDER")
