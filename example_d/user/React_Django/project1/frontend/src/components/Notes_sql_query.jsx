...
import List_sql_query from './List_sql_query';
//https://www.samuelliedtke.com/blog/react-table-tutorial-part-1/

function Note() {
  const [formNote, setFormNote] = useState({order_id: "", order_id_to_close: ""})
  ...
  function createNote(event) { //event es el evento producido aca <button onClick={createNote}>Create Post</button>
    axios({
      ...
      data:{
        ...
        order_id_to_close: formNote.order_id_to_close
       }
    })
    .then((response) => {
  ...
  <div className="note">
        <h1>sql_query</h1>
        <List_sql_query columns={columns} data={data} />
        ...
        <form className="note">
          <table>
            <tr>
              <td>order_id</td>
              <td>
                <input onChange={handleChange}  name="order_id" placeholder="order_id enter" value={formNote.order_id} />
              </td>
            </tr>
            <tr>
              <td>order_id_to_close</td>
              <td>
                <input onChange={handleChange} name="order_id_to_close" placeholder="order_id_to_close enter" value={formNote.order_id_to_close} />
              </td>
            </tr>
          </table>
